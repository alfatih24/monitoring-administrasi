...
?>
<div class="container">
	<form role="form" method="post" class="form-horizontal margin-top" enctype="multipart/form-data" action="default.php?p=modul/jurnal/ambiljurnal">
	<fieldset>
	  ...
	  <?php 
	  	...
	  	foreach ($alat as $alatdibawa) {
	  		...
	  		if($nama_alat == "Jurnal")
	  		{
	  			...
	  			$inpcheck = '<input type="checkbox" name="jurnal_required" value="'.$id_alat.'" '.$required.'>'.$nama_alat;
	  		}
	  		else 
	  		{
	  			$required 	= 'onclick="displayResult(this.form)"';
	  			$inpcheck	= '<input type="checkbox" name="list_barang" value="'.$id_alat.'" '.$required.'>'.$nama_alat;
	  		}
	  		echo '
			  <div class="checkbox">
...
<?php
	...
	{		
		...
		$barang 	= "1,".$_POST[selected_barang];
		if(substr($barang, -1) == ",")
		{
